...
class SystemServer(object):
    ...
    def rpc_get(self, session, rpc, filter_or_none):  # pylint: disable=W0613
        ...
        data = etree.parse('./yang/data/acc-devm-data.xml').getroot()
...
def main(*margs):
    # logging 使用方法： https://cloud.tencent.com/developer/article/1354396
    debug = True
    level=logging.DEBUG if debug else logging.INFO
    logging.basicConfig(level=level, filename='server.log', filemode='w')

    host_key = os.path.dirname(__file__) + "/server-key"
    auth = server.SSHUserPassController(username='admin', password='123')
